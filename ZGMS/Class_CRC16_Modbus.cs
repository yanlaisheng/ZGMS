...
namespace WNMS2
{
    public class Class_CRC16_Modbus
    {
        public static byte[] Crc16(byte[] frameBuffer, int len)
        {
            byte Crc16Hi = 0xff;
            byte Crc16Lo = 0xff;
            int i;
            ...
            for (i = 0; i < len; i++)
            {
                iIndex = Crc16Lo ^ frameBuffer[i];
                Crc16Lo = (byte)(Crc16Hi ^ GetCRCLo[iIndex]);
